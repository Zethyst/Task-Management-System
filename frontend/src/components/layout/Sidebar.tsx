...
export default function Sidebar({ isMobile }: { isMobile: boolean }) {
  ...
  return (
    <aside className={cn(
      "h-screen flex flex-col bg-gradient-to-b from-emerald-50 to-green-50/50 border-r border-emerald-200/50 fixed left-0 top-0 shadow-lg transition-all duration-300",
      isMobile ? "w-20" : "w-64"
    )}>
      {/* Logo */}
      ...
      <nav className="flex-1 p-4 space-y-1">
        {navItems.map(item => (
          <NavLink
            ...
            className={({ isActive }) => cn(
              ...
              'hover:bg-emerald-100/70 text-emerald-700 hover:shadow-sm',
        ...
        >
          ...
          {!isMobile && <span className="font-medium">Notifications</span>}
        </NavLink>
      ...
      <div className="p-4 border-t border-emerald-200/50 bg-white/50">
        <div className={cn(
          "flex items-center gap-3 mb-3 p-2 rounded-lg hover:bg-emerald-50/50 transition-colors duration-200",
          isMobile && "justify-center"
        )}>
          <Avatar className="h-10 w-10 ring-2 ring-emerald-200 flex-shrink-0">
        ...
        <Button 
          variant="ghost" 
          className={cn(
            "cursor-pointer w-full text-emerald-600 hover:text-red-600 hover:bg-red-50 transition-all duration-200 rounded-lg",
            isMobile ? "justify-center px-2" : "justify-start"
          )}
